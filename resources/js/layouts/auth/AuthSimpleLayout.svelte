...
<div
    class={`relative flex min-h-svh flex-col items-center justify-center overflow-hidden bg-[#f5f3eb] text-[#17201f] ${compactLogin ? 'p-3 md:p-4' : 'p-4 md:p-8'}`}
>
    <div
    ...
    ></div>
    <div
        class={`relative z-10 w-full ${wideLayout ? 'max-w-5xl' : compactLogin ? 'max-w-[23rem]' : 'max-w-[28rem]'}`}
    >
        <div class={`flex flex-col ${compactLogin ? 'gap-3' : 'gap-5'}`}>
            <div
                class={`flex flex-col items-center ${compactLogin ? 'gap-2' : 'gap-3'}`}
            >
                <Link
                    href={home()}
                ...
                >
                    <img
                        ...
                        alt="OptiBus"
                        class={`mb-1 h-auto object-contain drop-shadow-[0_12px_26px_rgba(16,61,58,0.12)] transition duration-300 group-hover:-translate-y-0.5 ${compactLogin ? 'w-[168px]' : 'w-[210px]'}`}
                        loading="eager"
                        decoding="async"
                    />
                ...
                {#if title || description}
                    <div class="max-w-2xl space-y-2 text-center">
                        {#if title}
                            <h1
                                class={`font-semibold tracking-[-0.03em] text-[#132b28] ${compactLogin ? 'text-xl md:text-2xl' : 'text-2xl md:text-3xl'}`}
                            >
                                {title}
                            </h1>
                        {/if}
                        {#if description}
                            <p
                                class={`text-center text-[#53615d] ${compactLogin ? 'text-xs leading-5' : 'text-sm leading-6'}`}
                            >
                                {description}
                            </p>
                        {/if}
    ...
    <div
        class={`pointer-events-none absolute left-1/2 hidden -translate-x-1/2 rounded-full border border-[#d9ded4]/80 bg-white/60 px-3 py-1 font-medium text-[#687470] shadow-sm backdrop-blur md:block ${compactLogin ? 'bottom-3 text-[10px]' : 'bottom-4 text-[11px]'}`}
    >
