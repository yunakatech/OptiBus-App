...
{:else if charterViewData}
    ...
    >
        <div
            class="relative border-b border-border/70 bg-[radial-gradient(circle_at_top_left,_rgba(34,211,238,0.18),_transparent_42%),linear-gradient(135deg,_rgba(15,23,42,0.98),_rgba(8,47,73,0.92))] px-5 py-5 text-white"
        >
            <div
                class="absolute inset-y-0 right-0 hidden w-64 bg-[radial-gradient(circle_at_center,_rgba(255,255,255,0.14),_transparent_62%)] lg:block"
            ></div>
            <div
                class="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"
            >
                <div class="space-y-3">
                    <div class="flex flex-wrap items-center gap-2">
                        <span
                            class={`rounded-full px-3 py-1 text-[11px] font-semibold ${charterStatusClass(charterViewData.status)}`}
                        >
                            {charterStatusLabel(charterViewData.status)}
                        </span>
                        <span
                            class={`rounded-full px-3 py-1 text-[11px] font-semibold ${charterPaymentClass(charterViewData.payment_status)}`}
                        >
                            {charterViewData.payment_status ?? '-'}
                        </span>
                        <span
                            class="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[11px] font-semibold text-slate-100"
                        >
                    ...
                    <div>
                        <p
                            class="text-[11px] uppercase tracking-[0.18em] text-cyan-100/80"
                        >
                            Detail Carter
                        </p>
                        <h2 class="mt-2 text-2xl font-semibold tracking-tight">
                            {charterViewData.name}
                        </h2>
                        <p class="mt-1 max-w-2xl text-sm text-slate-200">
                            {[
                                charterViewData.company_name,
                                charterViewData.phone,
                            ]
                                .filter(Boolean)
                                .join(' / ') || 'Kontak charter belum tersedia'}
                        </p>
                    </div>
                </div>
            ...
            <p
                class="relative z-10 mt-3 max-w-2xl text-xs leading-relaxed text-slate-200/85"
            >
        ...
        <div class="space-y-5 p-5">
            <div class="grid gap-3 md:grid-cols-3 xl:grid-cols-4">
                <div class="rounded-lg border border-border/70 bg-muted/20 p-4">
                    ...
                    <p class="mt-2 text-base font-semibold text-foreground">
                        {charterViewData.start_date}
                    </p>
                    <p class="text-sm text-muted-foreground">
                        {charterViewData.end_date}
                    </p>
                    <p class="mt-2 text-sm font-medium text-foreground">
                        {charterViewData.departure_time
                            ? String(charterViewData.departure_time).slice(0, 5)
                            : '--:--'}
                    </p>
                </div>
                <div class="rounded-lg border border-border/70 bg-muted/20 p-4">
                    ...
                    <p class="mt-2 text-base font-semibold text-foreground">
                        {charterViewData.driver_name ?? '-'}
                    </p>
                    <p class="mt-1 text-sm text-muted-foreground">
                        {charterViewData.unit_category ??
                            'Kategori armada belum dipilih'}
                    </p>
                    <p class="text-sm text-muted-foreground">
                ...
                <div class="rounded-lg border border-border/70 bg-muted/20 p-4">
                    ...
                    <p class="mt-2 text-base font-semibold text-foreground">
                        {formatCurrencyId(charterViewData.price ?? 0)}
                    </p>
                    <p class="mt-1 text-sm text-muted-foreground">
                        BOP {formatCurrencyId(charterViewData.bop_price ?? 0)}
                    </p>
                    <p
                        class={`text-sm font-medium ${(charterViewData.price ?? 0) - (charterViewData.bop_price ?? 0) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                    >
                        Margin {formatCurrencyId(
                            (charterViewData.price ?? 0) -
                                (charterViewData.bop_price ?? 0),
                        )}
                    </p>
                    <p class="text-sm text-muted-foreground">
                        Sisa {formatCurrencyId(
                            charterPaymentRemaining(charterViewData),
                        )}
                    </p>
                </div>
                <div class="rounded-lg border border-border/70 bg-muted/20 p-4">
                    ...
                    <p class="mt-1 text-sm text-muted-foreground">
                        DP {formatCurrencyId(charterViewData.down_payment ?? 0)}
                    </p>
                </div>
            </div>
            ...
            <div class="grid gap-4 xl:grid-cols-[1.35fr_0.9fr]">
                ...
                >
                    <div
                        class="flex items-center justify-between gap-3 border-b border-border/70 pb-3"
                    >
                        <div>
                            <p
                            ...
                            >
                                Pickup ke titik antar
                            </h3>
                        </div>
                    </div>
                    <div class="mt-4 grid gap-3 md:grid-cols-2">
                        <div
                            class="rounded-lg border border-emerald-200/70 bg-emerald-50/70 p-4"
                        >
                            <p
                                class="text-[11px] uppercase tracking-[0.08em] text-emerald-700"
                            >
                                Titik Jemput
                            </p>
                            <p
                                class="mt-2 text-sm font-semibold leading-relaxed text-emerald-950"
                            >
                                {charterViewData.pickup_point ?? '-'}
                            </p>
                        </div>
                        <div
                            class="rounded-lg border border-cyan-200/70 bg-cyan-50/70 p-4"
                        >
                            <p
                                class="text-[11px] uppercase tracking-[0.08em] text-cyan-700"
                            >
                                Titik Antar
                            </p>
                            <p
                                class="mt-2 text-sm font-semibold leading-relaxed text-cyan-950"
                            >
                                {charterViewData.drop_point ?? '-'}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-lg border border-border/70 bg-muted/15 p-4">
                    ...
                    <div class="mt-4 space-y-3">
                        ...
                        >
                            ...
                            <span
                                class="text-right font-semibold text-foreground"
                                >{charterViewData.payment_status ?? '-'}</span
                            >
                        </div>
                    </div>
                    <div
                        class="mt-4 rounded-lg border border-border/70 bg-card p-4"
                    >
                        <p
                            class="text-[11px] uppercase tracking-[0.08em] text-muted-foreground"
                        >
                        ...
                        <p class="mt-1 text-sm text-muted-foreground">
                            Total {formatCurrencyId(charterViewData.price ?? 0)} |
                            DP {formatCurrencyId(
                                charterViewData.down_payment ?? 0,
                            )}
                        </p>
                    </div>
                </div>
