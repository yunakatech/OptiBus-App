<script lang="ts">
    ...
    const driverSummaryCards = $derived([
        {
            key: 'revenue',
            label: 'Total Revenue',
            valueText: formatCurrency(driverSummary.revenue),
    ...
    const driverChartBars = $derived.by(() =>
        [...drivers]
            .sort(
                (left, right) =>
            ...
            .map((row) => ({
                key: String(row.id),
                label: row.nama,
                subtitle: `${row.nopol ?? '-'} · ${row.pool_name ?? 'Semua Pool'}`,
...
<div class="space-y-4 rounded-lg border border-border/70 bg-background/95 p-4 shadow-sm">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div class="space-y-1">
            <h3 class="text-lg font-bold tracking-tight md:text-xl">Driver</h3>
        </div>
...
<RevenueChartTable
    title="Driver"
    subtitle="Revenue-heavy driver list. Desktop pakai chart table; mobile tetap kartu ringkas."
    badges={[
        { key: 'total', label: `${driverMeta.total} driver` },
        { key: 'period', label: selectedPeriodLabel() },
        { key: 'page', label: pageSummary() },
    ]}
    summaryCards={driverSummaryCards}
    chartBars={driverChartBars}
>
    {#snippet controls()}
        <div class="grid gap-2 xl:grid-cols-[minmax(0,1fr)_minmax(180px,220px)_auto]">
            <div class="min-w-0">
