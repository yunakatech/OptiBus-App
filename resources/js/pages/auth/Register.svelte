...
<script lang="ts">
    ...
    import { ArrowRight, Check, CheckCircle2, ChevronDown, CreditCard, Route, ShieldCheck, Timer } from 'lucide-svelte';
    ...
    const flowDescription = $derived(isTrialFlow
        ? 'Akun dibuat dengan paket Starter trial. Setelah masuk, upgrade dapat dilakukan dari menu SaaS.'
        : 'Setelah akun dibuat, invoice pending akan tersedia di halaman Langganan untuk dibayar dengan QRIS atau transfer.');
...
>
    {#snippet children({ errors, processing })}
        <div class="border-b border-border/70 bg-[linear-gradient(135deg,rgba(240,253,250,0.96),rgba(255,255,255,0.98))] p-4 dark:bg-[linear-gradient(135deg,rgba(6,78,59,0.22),rgba(15,23,42,0.98))] sm:p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div class="space-y-2">
                    <div class="inline-flex w-fit items-center gap-2 rounded-full border border-emerald-200 bg-white/80 px-3 py-1 text-[11px] font-semibold uppercase text-emerald-700 dark:border-emerald-400/20 dark:bg-slate-950/40 dark:text-emerald-200">
                        {#if isTrialFlow}<Timer class="h-3.5 w-3.5" />{:else}<CreditCard class="h-3.5 w-3.5" />{/if}
                        {flowLabel}
                    </div>
                    <div>
                        <h2 class="text-xl font-semibold tracking-tight text-foreground">{flowTitle}</h2>
                        <p class="mt-1 text-sm leading-6 text-muted-foreground">{flowDescription}</p>
                    </div>
                </div>
                <div class="grid grid-cols-3 gap-1.5 rounded-2xl border border-white/80 bg-white/75 p-1.5 text-[10px] font-medium text-muted-foreground shadow-sm dark:border-white/10 dark:bg-slate-950/35">
                    <div class="rounded-xl bg-emerald-50 px-2 py-2 text-center text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-100">
                        <ShieldCheck class="mx-auto mb-1 h-3.5 w-3.5" />
                        Tenant
                    </div>
                    <div class="rounded-xl bg-sky-50 px-2 py-2 text-center text-sky-700 dark:bg-sky-400/10 dark:text-sky-100">
                        <Route class="mx-auto mb-1 h-3.5 w-3.5" />
                        Rute
                    </div>
                    <div class="rounded-xl bg-amber-50 px-2 py-2 text-center text-amber-700 dark:bg-amber-400/10 dark:text-amber-100">
        ...
        <div class="grid gap-5 p-4 sm:p-5">
                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div class="min-w-0">
                                <div class="flex items-center gap-2">
                                    <span class="font-semibold text-foreground">{currentPlan?.name ?? 'Pilih Paket'}</span>
                                    {#if currentPlan?.slug === 'pro'}<Badge variant="default" class="py-0 text-[10px]">Populer</Badge>{/if}
                                </div>
                            ...
                            {#if isPaymentFlow}
                                <ChevronDown class="mt-1 h-4 w-4 shrink-0 text-muted-foreground" />
                            {/if}
                        </div>
                    </button>

                    {#if showPlanSelector && isPaymentFlow}
                        <div class="absolute z-10 mt-1 w-full overflow-hidden rounded-2xl border bg-background shadow-lg">
                            {#each plans as plan}
                                <button
                                    type="button"
                                ...
                                >
                                    <div class="min-w-0">
                                        <div class="flex items-center gap-2 font-semibold">
                                            {plan.name}
                                            {#if plan.slug === 'pro'}<Badge variant="default" class="py-0 text-[10px]">Populer</Badge>{/if}
                                        </div>
                                        <p class="mt-1 text-sm leading-5 text-muted-foreground">{plan.description}</p>
                                    </div>
                                    <div class="shrink-0 text-right">
                                        <div class="font-semibold">{formatRupiah(plan.price_monthly)}<span class="text-xs font-normal text-muted-foreground">/bln</span></div>
                                        {#if selectedPlan === plan.slug}<Check class="mt-1 ml-auto h-4 w-4 text-primary" />{/if}
                                    </div>
                                </button>
                            {/each}
                        </div>
            ...
            <section class="grid gap-4 rounded-2xl border border-border/70 p-3 sm:p-4">
                <div>
                    <h3 class="text-sm font-semibold text-foreground">Data Travel</h3>
                    <p class="mt-0.5 text-xs text-muted-foreground">Dipakai untuk membuat tenant, pool awal, dan rute utama.</p>
                </div>
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div class="grid gap-2">
                        <Label for="origin">Dari <span class="text-destructive">*</span></Label>
                        <Input id="origin" type="text" required name="origin" placeholder="Contoh: Pinrang" />
                        <InputError message={errors.origin} />
                    </div>
                    <div class="grid gap-2">
                        <Label for="destination">Tujuan <span class="text-destructive">*</span></Label>
            ...
            <section class="grid gap-4 rounded-2xl border border-border/70 p-3 sm:p-4">
                <div>
                    <h3 class="text-sm font-semibold text-foreground">Akun Owner</h3>
                    <p class="mt-0.5 text-xs text-muted-foreground">Akun ini menjadi admin awal tenant.</p>
                </div>
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="grid gap-2">
            ...
            <Button type="submit" class="h-11 w-full rounded-2xl" disabled={processing}>
            ...
            {#if isTrialFlow}
                ...
                <a href="/auth/google/redirect" class="w-full">
                    <Button type="button" variant="outline" class="h-11 w-full rounded-2xl gap-2">
