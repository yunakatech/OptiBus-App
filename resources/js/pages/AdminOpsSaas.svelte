...
<script lang="ts">
    ...
    function openTenantForm(tenant?: any) {
        if (tenant) {
            ...
            tenantForm = {
                ...
                plan_id: tenant.subscription_id ? 0 : 1,
                trial_days: 14,
            };
        } else {
            editingTenant = null;
    ...
    function toggleTenantStatus(tenant: any) {
        ...
        apiFetch('/api/admin/tenants', {
            ...
            if (d?.success) loadTenants(tenantPagination?.page ?? 1);
        });
    }
    ...
    async function saveSubscription() {
        ...
        if (editingSub) {
            ...
            body.tenant_id = subForm.tenant_id;
        }
        ...
        if (data?.success) {
            message = data.message;
            showSubForm = false;
            loadSubscriptions(subPagination?.page ?? 1);
        }
    }
    ...
    function startEditPlan(plan: any) {
        editingPlan = {
            ...plan,
            _features: plan.features ? [...plan.features] : [],
        };
    }
    ...
    async function markInvoicePaid(invoice: any) {
        ...
        const data = await apiFetch(
            `/api/admin/invoices/${invoice.id}/mark-paid`,
            {
                ...
                body: JSON.stringify({
                    payment_method:
                        invoice.payment_method || 'Admin Correction',
                }),
            },
        );
        if (data?.success) {
    ...
    function invoiceBadge(invoice: any): {
        ...
        if (invoice.status === 'paid')
            return { variant: 'default', label: 'Lunas' };
    ...
    function gatewayStatus(inv: any): string {
        const status = inv.gateway_status || inv.status || '-';
        const labels: Record<string, string> = {
            creating_link: 'Membuat link',
            payment_link_error: 'Link error',
            pending: 'Pending',
            paid: 'Paid',
            failed: 'Failed',
            expired: 'Expired',
            ...
            <Card class={panelClass}>
                <CardContent class="py-14 text-center">
                    ...
                    <h3 class="mt-4 text-lg font-semibold">
                        Tabel SaaS Belum Tersedia
                    </h3>
                    <p class="mt-1 text-sm text-muted-foreground">
                        Jalankan migrasi database dulu.
                    </p>
                </CardContent>
            </Card>
        {:else}
            ...
            {#if message}
                <div
                    class="flex items-center justify-between rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800 dark:border-emerald-900/50 dark:bg-emerald-950/40 dark:text-emerald-300"
                >
                    {message}
                    <button
                        onclick={() => (message = '')}
            ...
            <section class={shellClass}>
                <div class="relative isolate overflow-hidden rounded-[28px]">
                    ...
                    >
                        <div class="space-y-4">
                            <div
                                class="inline-flex items-center gap-2 rounded-full border border-slate-200/80 bg-white/80 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500 shadow-sm dark:border-slate-800 dark:bg-slate-950/60 dark:text-slate-400"
                            >
                            ...
                            <div class="space-y-2">
                                ...
                                >
                                    ...
                                    Semua state kebaca, semua kerjaan kepegang
                                    dari satu layar.
                                </p>
                            </div>
                            <div class="flex flex-wrap items-center gap-2">
                                {#if summary}
                                    <Badge variant="outline"
                                        >{summary.tenant_count} tenants</Badge
                                    >
                                    <Badge variant="secondary"
                                        >{summary.active_subscription_count} active</Badge
                                    >
                                    <Badge variant="outline"
                                        >{summary.plan_count} plans</Badge
                                    >
                                    <Badge variant="outline"
                                        >{summary.invoice_pending_count ?? 0} pending</Badge
                                    >
                                    <Badge
                                        variant={(summary.invoice_overdue_count ??
                                            0) > 0
                                            ? 'destructive'
                                            : 'outline'}
                                        >{summary.invoice_overdue_count ?? 0} overdue</Badge
                                    >
                                {/if}
                                <Badge
                                    variant={saasTablesReady
                                        ? 'default'
                                        : 'outline'}
                                    >{saasTablesReady
                                        ? 'tables ready'
                                        : 'migrasi needed'}</Badge
                                >
                            </div>
                        </div>
                    ...
                    >
                        <div class="space-y-1">
                            ...
                            <h2 class="text-lg font-semibold text-foreground">
                                Kelola workspace tenant
                            </h2>
                            <p class="text-sm text-muted-foreground">
                                Cari, tambah, suspend, atau cek subscription
                                dari satu panel.
                            </p>
                        </div>
                        ...
                        <Card class={panelClass}>
                            ...
                            <CardContent>
                                ...
                                >
                                    ...
                                    {#if !editingTenant}
                                        <div>
                                            ...
                                            <select
                                                bind:value={tenantForm.plan_id}
                                                class="w-full rounded-md border px-3 py-2 text-sm"
                                            >
                                                {#each tenantPlans as plan}
                                                    <option value={plan.id}
                                                        >{plan.name} ({formatRupiah(
                                                            plan.price_monthly,
                                                        )})</option
                                                    >
                                                {/each}
                                ...
                                <div class="flex justify-end gap-2 mt-4">
                                    <Button
                                        variant="outline"
                                        onclick={() => (showTenantForm = false)}
                                        >Batal</Button
                                    >
                                    <Button onclick={saveTenant} disabled={busy}
                                        >{busy
                                            ? 'Menyimpan...'
                                            : 'Simpan'}</Button
                                    >
                                ...
                                <table class="w-full text-sm">
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                            <th
                                                class="px-4 py-3 font-semibold text-muted-foreground text-right"
                                                >Users</th
                                            >
                                            <th
                                                class="px-4 py-3 font-semibold text-muted-foreground text-right"
                                    ...
                                    <tbody>
                                        {#each tenants as t}
                                            <tr
                                                class="border-b border-slate-100 last:border-0 hover:bg-slate-50/70 dark:border-slate-800 dark:hover:bg-slate-900/40"
                                            >
                            ...
                            {#if tenantPagination?.last_page > 1}
                                ...
                                >
                                    <span class="text-xs text-muted-foreground"
                                        >Page {tenantPagination.page} of {tenantPagination.last_page}
                                        ({tenantPagination.total} total)</span
                                    >
                                    <div class="flex gap-1">
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            disabled={tenantPagination.page <=
                                                1}
                                            onclick={() =>
                                                loadTenants(
                ...
                <section class="space-y-4">
                    ...
                    >
                        <div class="space-y-1">
                            ...
                            <h2 class="text-lg font-semibold text-foreground">
                                Kelola lifecycle langganan
                            </h2>
                            <p class="text-sm text-muted-foreground">
                                Filter status, ubah plan, dan kontrol tenant
                                dari satu daftar.
                            </p>
                        </div>
                        <div class="flex flex-wrap items-center gap-3">
                            ...
                            <CardContent>
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <Input
                                            type="date"
                                            bind:value={subForm.ends_at}
                                        />
                                    </div>
                                    <div class="md:col-span-2">
                                        <Label>Catatan</Label>
                                        <Input
                                            bind:value={subForm.notes}
                                            placeholder="opsional"
                                        />
                                    </div>
                                </div>
                    ...
                    <Card class={tableShellClass}>
                        <CardContent class="p-0">
                            ...
                            {#if subPagination?.last_page > 1}
                                ...
                                >
                                    <span class="text-xs text-muted-foreground"
                                        >Page {subPagination.page} of {subPagination.last_page}</span
                                    >
                                    <div class="flex gap-1">
                                        <Button
                                            variant="outline"
                                        ...
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            disabled={subPagination.page >=
                                                subPagination.last_page}
                                            onclick={() =>
                                                loadSubscriptions(
                                                    subPagination.page + 1,
                                                )}>Next</Button
                ...
                <section class="space-y-4">
                    ...
                    >
                        ...
                        <h2 class="mt-1 text-lg font-semibold text-foreground">
                            Atur paket dan limit produk
                        </h2>
                        <p class="text-sm text-muted-foreground">
                            Harga, limit resource, dan fitur premium dipelihara
                            di sini.
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                        {#each plans as plan}
                            ...
                            <Card
                                class={`${panelClass} ${plan.is_active ? '' : 'opacity-70'}`}
                            >
                                <CardHeader>
                                    <div
                                    ...
                                    {:else}
                                        ...
                                        <div class="pt-2">
                                            <Button
                                                ...
                                                onclick={() =>
                                                    startEditPlan(plan)}
                                                ><Edit3 class="h-3 w-3 mr-1" /> Edit</Button
                                            >
                                        </div>
                                    {/if}
                                </CardContent>
            ...
            {#if activeTab === 'billing'}
                <section class="space-y-4">
                    ...
                    >
                        ...
                        <h2 class="mt-1 text-lg font-semibold text-foreground">
                            Invoice subscription dan gateway Mayar
                        </h2>
                        <p class="text-sm text-muted-foreground">
                            Pantau checkout URL, status gateway, dan invoice
                            yang butuh aksi.
                        </p>
                    </div>
                    ...
                    <Card class={tableShellClass}>
                        <CardContent class="p-0">
                            ...
                            {#if invoicePagination?.last_page > 1}
                                ...
                                >
                                    <span class="text-xs text-muted-foreground"
                                        >Page {invoicePagination.page} of {invoicePagination.last_page}
                                        ({invoicePagination.total} total)</span
                                    >
                                    <div class="flex gap-1">
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            disabled={invoicePagination.page <=
                                                1}
                                            onclick={() =>
                                                loadInvoices(
            ...
            {#if activeTab === 'payment'}
                <section class="space-y-4">
                    ...
                    >
                        ...
                        >
                            <div>
                                ...
                                <p class="text-sm text-muted-foreground">
                                    Mayar menjadi satu-satunya checkout
                                    user-facing untuk SaaS billing.
                                </p>
                            </div>
                            <Badge
                                variant={paymentSettings?.enabled &&
                    ...
                    {#if payMessage}
                        <div
                            class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800 dark:border-emerald-900/50 dark:bg-emerald-950/40 dark:text-emerald-300"
                        >
                            {payMessage}
                        </div>
                    {/if}
                    ...
                    <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
                        <Card class={panelClass}>
                            <CardHeader>
                                ...
                                >
                                    <div>
                                        <CardTitle>Konfigurasi Mayar</CardTitle>
                                        <p
                                            class="mt-1 text-xs text-muted-foreground"
                                        >
                                            Nilai ini dibaca dari environment
                                            server, bukan dari form admin.
                                        </p>
                                    </div>
                                    <Badge
                                        variant={paymentSettings?.enabled
                            ...
                            <CardContent class="space-y-3 text-sm">
                                ...
                                >
                                    ...
                                    <p
                                        class="mt-2 break-all font-mono text-xs text-foreground"
                                    >
                                        {paymentSettings?.api_url}{paymentSettings?.payment_create_path}
                                    </p>
                                </div>
                        ...
                        <Card class={panelClass}>
                            <CardHeader>
                                <CardTitle>Lifecycle SaaS</CardTitle>
                            </CardHeader>
                            <CardContent
                            ...
                            >
                                ...
                                >
                                    ...
                                    <p class="mt-1">
                                        Invoice subscription dibuat di OptiBus,
                                        lalu sistem membuat checkout URL Mayar.
                                    </p>
                                </div>
                                <div
                                    class="rounded-2xl border border-slate-200/70 p-4 dark:border-slate-800"
                                >
                                    ...
                                    <p class="mt-1">
                                        Mayar mengirim webhook, invoice ditandai
                                        lunas, subscription dan tenant aktif.
                                    </p>
                                </div>
                                <Button
                                    variant="outline"
